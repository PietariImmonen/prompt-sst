        ...
        <SidebarMenuButton className="pl-0 hover:cursor-pointer data-[state=open]:bg-sidebar data-[state=open]:text-foreground">
          <Avatar
            className={cn(
              ...
              "h-7 w-7 rounded-full p-[2px] hover:bg-background hover:text-foreground",
            )}
          >
            <AvatarFallback className="p-0">
              <p className="text-sm font-semibold text-secondary-foreground">
                {auth.current.name[0].toUpperCase()}
              </p>
            </AvatarFallback>
          </Avatar>
          <p className="flex-1 truncate text-start text-xs font-medium text-foreground">
            {auth.current.name}
          </p>
          <MoreHorizontal className="size-4 opacity-50" />
          <span className="sr-only">
            {t("nav:description.toggleAccountMenu")}
          </span>
        </SidebarMenuButton>
      </DropdownMenuTrigger>
